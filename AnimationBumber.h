...
void HorizontalRigthLine(boolean OFF,byte matrix, byte row, byte column, byte length_, int ping){ //OFF - ��������, ��������� �������
  for(int i=0;i<length_;i++){
    lc.setLed(matrix,column,i+row,OFF);
    delay(ping);
  }
}
 void HorizontalLeftLine(boolean OFF,byte matrix, byte row, byte column, byte length_, int ping){ //OFF - ��������, ��������� �������
  for(int i=length_-1;i>=0;i--){
    lc.setLed(matrix,column,i+row,OFF);
    delay(ping);
  }
}
 void VerticalUpLine(boolean OFF,byte matrix, byte row, byte column, byte length_, int ping){ //OFF - ��������, ��������� �������
  for(int i=length_-1;i>=0;i--){
    lc.setLed(matrix,column+i,row,OFF);
    delay(ping);
  }
}
 void VerticalDownLine(boolean OFF,byte matrix, byte row, byte column, byte length_, int ping){ //OFF - ��������, ��������� �������
  for(int i=0;i<length_;i++){
    lc.setLed(matrix,column+i,row,OFF);
    delay(ping);
  }
